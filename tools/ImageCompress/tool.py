...
def list_img_file(directory):
    new_list = []
    len_dir = int(len(directory))
    """列出目录下所有文件，并筛选出图片文件列表返回"""
    for root, dirs, files in os.walk(directory):
        for file_single in files:
            file_name = os.path.join(root, file_single)[len_dir:]
            name, fileformat = file_name.split(".")
            if fileformat.lower() == "jpg" or fileformat.lower() == "png" or fileformat.lower() == "gif":
                new_list.append(file_name)
        if directory == "photos/":
            for dir_single in dirs:
                dir_name = os.path.join(root, dir_single)[len_dir:]
                path = "source/static/" + dir_name
...
def compress_photo():
    '''调用压缩图片的函数
    '''
    src_dir, des_dir = "photos/", "source/static/"
...
if __name__ == "__main__":
    ...
    compress_photo()   # 压缩图片，并保存到source/static文件夹下
